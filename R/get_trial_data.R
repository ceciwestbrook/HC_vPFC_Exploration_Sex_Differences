...
if (!exists("remap_ids")) source(file.path(repo_directory, "R/exclusions.R"))
...
get_trial_data <- function(repo_directory=NULL, dataset="mmclock_fmri", groupfixed=TRUE) {
  ...
  if (dataset=="mmclock_fmri") {
    trials_per_run <- 50
    full <- read_csv(file.path(repo_directory, "trial_data/mmclock_fmri_fixed_fixedparams_fmri_ffx_trial_statistics.csv.gz"))
    u_df <- read_csv(file.path(repo_directory, "trial_data/mmclock_fmri_fixed_uv_ureset_fixedparams_fmri_ffx_trial_statistics.csv.gz"))
    
    if (isTRUE(groupfixed)) {
      trial_df <- read.csv(file.path(repo_directory, "trial_data/mmclock_fmri_decay_factorize_selective_psequate_fixedparams_ffx_trial_statistics.csv.gz"))
    } else {
      trial_df <- read_csv(file.path(repo_directory, "trial_data/mmclock_fmri_decay_factorize_selective_psequate_mfx_trial_statistics.csv.gz"))
    }
    ...
    if (isTRUE(groupfixed)) {
      trial_df <- read.csv(file.path(repo_directory, "trial_data/explore_decay_factorize_selective_psequate_fixedparams_fmri_ffx_sceptic_trial_statistics.csv")) %>%
    ...
    # Two participants were entered under incorrect ids and are remapped here.
    # The mapping lives with the data rather than in this repository, for the
    # same reason as the exclusion list. See R/exclusions.R and id_remap_TEMPLATE.csv.
    trial_df$id <- remap_ids(trial_df$id)
  ...
  # params <- read_csv(file.path(repo_directory, "trial_data/mmclock_fmri_decay_factorize_selective_psequate_mfx_sceptic_global_statistics.csv")) %>%
  #   dplyr::select(-model)
  # 
